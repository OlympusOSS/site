...
If `SESSION_SIGNING_KEY` is not set in the container, Athena fails to start with a fatal error. See [session-signing-key.md](/docs/operate/rotation/session-signing-key-rotation) for the full startup validation behavior and fix command. Verify with `GET /api/health`, if it returns `200`, the environment is configured correctly.

Note: `ENCRYPTION_KEY` is a separate key used for SDK settings encryption. Missing `ENCRYPTION_KEY` affects settings decryption, not session authentication. See [session-signing-key.md](/docs/operate/rotation/session-signing-key-rotation) for key separation details.
...
- [athena#99](https://github.com/OlympusOSS/athena/issues/99), SESSION_SIGNING_KEY separation from ENCRYPTION_KEY (see [session-signing-key.md](/docs/operate/rotation/session-signing-key-rotation))
