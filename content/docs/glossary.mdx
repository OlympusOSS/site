...
**Audit log** — The `security_audit` table in the `olympus` database. Recorded events: login success, login failure, lockout, lockout release, password change, settings change. See [Security — Brute-Force Protection](/docs/security/brute-force).

## B

**Breached password check** — k-anonymity lookup against HaveIBeenPwned at registration and password change. See [Security — Breached Password](/docs/security/breached-password).

**Brute-force protection** — Per-identifier login-attempt rate limiting that escalates to an account lockout on repeated failures. Implemented in the SDK. See [Security — Brute-Force](/docs/security/brute-force).

## C

**Caddy** — The reverse proxy that fronts Olympus. Terminates TLS, applies rate limits, sets security headers. See [Security — Caddy Supply Chain](/docs/security/caddy-supply-chain).
...
**Captcha** — Bot-mitigation challenge applied to registration, login, and recovery flows. Olympus uses Cloudflare Turnstile. Disabled by default in dev. See [Security — Captcha Turnstile](/docs/security/captcha-turnstile).
...
**Client credentials grant** — OAuth2 grant for server-to-server (M2M) calls where no human user is involved. See [Integrate — Client Credentials](/docs/integrate/oauth2-client-credentials).
...
**Cookie secret** — The HMAC key used by Hydra to sign session cookies. Rotatable. Listed in [Operate — Secrets Audit](/docs/operate/secrets-audit).
...
**CSP (Content Security Policy)** — HTTP header restricting what resources a page can load. Both Athena and Hera ship hardened CSPs. See [Security — CSP Athena](/docs/security/csp-athena) and [Security — CSP Hera](/docs/security/csp-hera).
...
**DBA account** — A per-engineer database role mapped from an OIDC role claim. See [Security — pgAdmin DBA Accounts](/docs/security/pgadmin-dba-accounts).
...
**Encryption key** — A 32-byte secret used by the SDK for AES-256-GCM encryption of sensitive settings values. Rotation procedure: see [Operate — Encryption Key Rotation](/docs/operate/encryption-key-rotation).

**Encryption key blocklist** — A list of known-weak / known-public values the SDK refuses to accept as `ENCRYPTION_KEY`. See [Security — Encryption Key Blocklist](/docs/security/encryption-key-blocklist).
...
**HKDF** — HMAC-based Key Derivation Function. The SDK uses HKDF-SHA256 to derive per-record encryption keys from the master `ENCRYPTION_KEY`. See [Security — Encryption at Rest](/docs/security/encryption-at-rest).
...
**Lockout** — A temporary or permanent block on a specific identifier's ability to log in, following repeated failed login attempts. Stored in the `lockouts` table. See [Security — Account Lockout](/docs/security/account-lockout).
...
**M2M (Machine-to-Machine)** — Server-to-server OAuth2, using the `client_credentials` grant. Olympus supports M2M against Hydra. See [Integrate — Client Credentials](/docs/integrate/oauth2-client-credentials).

**MCP (Model Context Protocol)** — The Anthropic-defined protocol for exposing tools to LLMs. Daedalus embeds an MCP server on `127.0.0.1:14210`. See [Internals — Daedalus MCP Server](/docs/internals/daedalus-mcp-server).
...
**Network topology** — The set of host-bound vs intranet-only ports in the Compose stack. See [Operate — Network Topology](/docs/operate/network-topology).
...
**PKCE (Proof Key for Code Exchange)** — RFC 7636 extension to OAuth2 Authorization Code that prevents authorization-code interception attacks. **Mandatory for all public clients in Olympus.** See [Security — PKCE Enforcement](/docs/security/pkce-enforcement).
...
**Reload API key** — The shared secret between Kratos and its schema-reload sidecar that authorizes live schema reloads. See [Operate — Reload API Key Rotation](/docs/operate/reload-api-key-rotation).
...
**Session signing key** — The HMAC-SHA256 key used by Athena to sign its session cookie. Separate from `ENCRYPTION_KEY`. See [Operate — Session Signing Key Rotation](/docs/operate/session-signing-key-rotation).
...
**verify-full** — A PostgreSQL TLS mode that verifies both the certificate chain and the server hostname. Required in Olympus production. See [Deploy — Database SSL verify-full](/docs/deploy/database-ssl-verify-full).
