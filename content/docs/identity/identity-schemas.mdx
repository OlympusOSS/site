...
Email is verified via the standard Kratos verification flow. The `email_verified` claim that comes from a social IdP is **not trusted**, Kratos always runs its own verification. See [Security, OIDC `email_verified` trust](/docs/security/identity-protection/oidc-email-verified-trust).
...
4. Run schema reload via the sidecar (see [Operate, Reload API Key Rotation](/docs/operate/rotation/reload-api-key-rotation) for how the sidecar works).
...
A Kratos config or schema change requires Kratos to reload. The platform has a sidecar that listens on `RELOAD_API_KEY` and triggers a SIGHUP to Kratos. See [Operate, Reload API Key Rotation](/docs/operate/rotation/reload-api-key-rotation).
