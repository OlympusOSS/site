...
Recovery isn't a login — the brute-force lockout doesn't gate it. (Brute-force protection at the email-sending layer is different — see [Security — Rate Limiting](/docs/security/rate-limiting) for Caddy-level rate-limit on `/self-service/recovery`.)
