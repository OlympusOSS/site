...
See [Security, Brute-Force Protection](/docs/security/web-attacks/brute-force).

### Captcha

If captcha is required (configurable per deployment), Hera validates the Turnstile token with Cloudflare before passing the submission to Kratos. See [Security, Captcha Turnstile](/docs/security/web-attacks/captcha-turnstile).
...
| Locked out | "Too many attempts" | The identifier is locked. See [Operate, Locked Account Unlock](/docs/operate/backups-recovery/locked-account-unlock). |
...
- [Troubleshooting, Login loops](/docs/troubleshooting/auth-issues/login-loops), most common Olympus login problem.
