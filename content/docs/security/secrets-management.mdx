...
| --- | --- | --- | --- |
| `ENCRYPTION_KEY` | SDK — settings encryption | GitHub Secrets → compose env | yes, [runbook](/docs/operate/encryption-key-rotation) |
| `SESSION_SIGNING_KEY` | Athena — session HMAC | GitHub Secrets → compose env | yes, [runbook](/docs/operate/session-signing-key-rotation) |
| `CIAM_RELOAD_API_KEY`, `IAM_RELOAD_API_KEY` | Kratos schema reload sidecar | GitHub Secrets → compose env | yes, [runbook](/docs/operate/reload-api-key-rotation) |
...
| Postgres CA cert | Every service, SSL verify-full | GitHub Actions artifact or static file | yes, [runbook](/docs/operate/cert-rotation) |
...
Social IdP secrets, SMTP credentials, etc. are stored in the `olympus` database as encrypted settings. The SDK encrypts using `ENCRYPTION_KEY` (compose env) with HKDF-derived per-record keys. See [Security — Encryption at Rest](/docs/security/encryption-at-rest).
...
Every quarter, run the [Secrets Audit runbook](/docs/operate/secrets-audit). Checklist:
