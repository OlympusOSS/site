...
[Cookbook — Force MFA by role](/docs/cookbook/force-mfa-by-role) makes MFA mandatory for IAM admins.
...
In Athena, sensitive operations (creating M2M clients, viewing audit log, deleting identities) should require AAL2 — see [Cookbook — Enforce step-up auth](/docs/cookbook/enforce-step-up-auth).

Even if attacker has a recently-compromised AAL1 session, they can't immediately escalate.

### Notify on new device sign-in

If the user's email is BEC'd and the attacker logs into Olympus from a new device, the [new-device email notification](/docs/cookbook/notify-new-device) goes to the user's real email (assuming the BEC'd email is the only one) — but the attacker can intercept.
...
2. Revoke all sessions for that identity ([Operate — Incident response](/docs/operate/incident-response)).
