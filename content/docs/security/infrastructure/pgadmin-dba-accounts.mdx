...
1. Verify the DBA has an IAM Kratos identity with `"dba"` in their `roles` trait. See the
   provisioning steps in [pgadmin-access.md](/docs/security/infrastructure/pgadmin-sso).
...
Follow the offboarding runbook at [runbook-pgadmin-dba-offboarding.md](/docs/operate/administration/pgadmin-dba-offboarding).
