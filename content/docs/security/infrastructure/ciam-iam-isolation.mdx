...
The compensating controls are: harden the host (firewall, SSH key auth, fail2ban), rotate `ENCRYPTION_KEY` quarterly, pin Caddy image by digest, run [Security, Caddy Supply Chain](/docs/security/infrastructure/caddy-supply-chain) builds.
...
- [Operate, Network Topology](/docs/operate/administration/network-topology), port-by-port.
- [Security, Threat Model](/docs/security/infrastructure/threat-model), where isolation matters.
