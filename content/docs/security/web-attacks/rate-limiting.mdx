...
- [caddy-supply-chain.md](/docs/security/infrastructure/caddy-supply-chain), version pinning, post-build smoke test, SHA-tagged image releases, and proxy topology configuration for the Caddy layer
- [kratos-production-config.md](/docs/deploy/production/kratos-production-config), required Kratos production configuration including `leak_sensitive_values: false`
