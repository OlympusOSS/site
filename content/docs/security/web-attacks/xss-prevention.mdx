...
Both Hera and Athena ship with [Content Security Policy](/docs/security/web-attacks/csp-hera) that blocks inline scripts and limits script sources to first-party. This dramatically reduces the impact of any HTML injection, even if a vulnerability existed, the injected `<script>` wouldn't execute.
...
[See SPA integration](/docs/integrate/spa/spa-integration) for details.
