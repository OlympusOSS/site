...
- Offboarding: `state: deactivated` or hard delete. See [Operate, pgAdmin DBA offboarding](/docs/operate/administration/pgadmin-dba-offboarding) as a template.
...
- See [Operate, Network topology](/docs/operate/administration/network-topology) and [Security, CIAM/IAM isolation](/docs/security/infrastructure/ciam-iam-isolation).

### CC6.4-6.8, Logical access maintenance

- Quarterly access review: see [Operate, Secrets audit](/docs/operate/monitoring/secrets-audit), apply same cadence to identity audit.
- Privileged access (admin role), every admin operation logged.

### CC7.1, System monitoring

- Audit log (`security_audit` table), see [Operate, Audit log retention](/docs/operate/monitoring/audit-log-retention).
- Health endpoints, see [Operate, Health endpoints](/docs/operate/monitoring/health-endpoints).
...
- Optional: impossible travel detection ([Cookbook](/docs/cookbook/defensive-security/detect-impossible-travel)).

### CC7.3-7.4, Incident response

- Documented playbook: [Operate, Incident response](/docs/operate/backups-recovery/incident-response).
...
- Postgres backups: [Operate, Backups (Postgres)](/docs/operate/backups-recovery/backups-postgres).
...
- Encryption key rotation ([Operate](/docs/operate/rotation/encryption-key-rotation)).
...
- [ ] All secrets rotated within the past quarter ([Operate, Secrets audit](/docs/operate/monitoring/secrets-audit)).
