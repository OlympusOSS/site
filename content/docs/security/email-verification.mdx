...
See [`docs/oidc-email-verified-trust-decision.md`](/docs/security/oidc-email-verified-trust) for the full architectural decision record. The short summary: Option B (always require Kratos verification) is the **current deliberate choice**, not a default that was overlooked.
