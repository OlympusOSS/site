...
| Athena `/api/health` 200 but every route 500 | Likely `SESSION_SIGNING_KEY` rotation went wrong. See [incident response](/docs/operate/incident-response). |

## Where next

- [Operate — Incident Response](/docs/operate/incident-response) — full on-call playbook.
- [Operate — Network Topology](/docs/operate/network-topology) — what's host-bound vs internal.
- [Operate — Logs and Observability](/docs/operate/logs-and-observability) — how to read logs for failure modes.
