...
- **Quarterly** as part of the [Secrets Audit](/docs/operate/monitoring/secrets-audit) cadence.
- **Immediately** if the key has been exposed (env var leaked in a log, in a screenshot, in a chat, on a former employee's laptop).
- **After every significant change** to the SDK's encryption format (see [Security, Encryption at Rest](/docs/security/encryption-keys/encryption-at-rest) for the format-version table).
...
- Generate the new key:
  ...
  Verify it doesn't match anything on the [Encryption Key Blocklist](/docs/security/encryption-keys/encryption-key-blocklist), the SDK will refuse to start with a blocklisted value, but it's faster to check before deploy.
...
- [Security, Encryption at Rest](/docs/security/encryption-keys/encryption-at-rest), the encryption format in detail.
...
- [Operate, Secrets Audit](/docs/operate/monitoring/secrets-audit), the quarterly cadence.
- [Operate, Session Signing Key Rotation](/docs/operate/rotation/session-signing-key-rotation), the parallel runbook for `SESSION_SIGNING_KEY`.
