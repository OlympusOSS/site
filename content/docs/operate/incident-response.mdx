...
| **Sev 4** | Cosmetic or pre-incident. Cert expires in <30 days, low disk warning, etc. | next business day |
...
If expired, run [Operate — Cert Rotation](/docs/operate/cert-rotation).
...
2. Rotate `ENCRYPTION_KEY` ([procedure](/docs/operate/encryption-key-rotation)).
3. Rotate `SESSION_SIGNING_KEY` ([procedure](/docs/operate/session-signing-key-rotation)).
...
- [Operate — Network Topology](/docs/operate/network-topology) — which ports must be reachable.
