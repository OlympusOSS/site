...
Same flow as Google, see [Cookbook, Add Google](/docs/cookbook/social-login/add-social-provider-google).

## Caveats

- **Email visibility:** users with hidden email on GitHub return a `noreply.github.com` address. Test how your flow handles these.
- **No `email_verified` claim** from GitHub. Kratos's own verification flow runs (which is good, see [Security, OIDC email_verified trust](/docs/security/identity-protection/oidc-email-verified-trust)).
