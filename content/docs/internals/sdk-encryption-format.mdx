...
- **v1:** legacy format used in early Olympus development. The SDK still decodes v1 for backwards-compat but writes only v2. To remove v1 support entirely, run the migration in [Operate — Encryption key rotation](/docs/operate/encryption-key-rotation) which upgrades all v1 rows to v2.
