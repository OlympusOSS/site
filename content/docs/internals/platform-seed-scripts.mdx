...
Schemas are loaded from `kratos.yml`'s `identity.default_schema_id` pointer. Changes to schema files require a reload — see [Operate — Reload API key rotation](/docs/operate/reload-api-key-rotation) for the sidecar mechanism.
...
For prod, no seed re-runs automatically. To re-seed prod (e.g. starting fresh), use the [destroy → re-create](/docs/deploy/with-daedalus-wizard) Daedalus flow.
