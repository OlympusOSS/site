...
Hera's pages share Kratos's session cookie scope (`ory_kratos_session`). The cookie domain must match, see [Security, Session cookies](/docs/security/identity-protection/session-cookies).
...
When the identity schema changes, Kratos reloads via the sidecar (see [Operate, Reload API key rotation](/docs/operate/rotation/reload-api-key-rotation)). Hera doesn't need a reload, it re-fetches the schema with every flow.

## Related

- [Internals, Hera, Route map](/docs/internals/hera/hera-route-map)
- [Internals, Hera, Hydra integration](/docs/internals/hera/hera-hydra-integration)
