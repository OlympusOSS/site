...
The MCP server is part of the Rust backend, spawned as a Tokio task in `mcp::mod::start()`. See [Internals, Daedalus MCP server](/docs/internals/daedalus/daedalus-mcp-server).
...
- [Internals, Daedalus PTY pipeline](/docs/internals/daedalus/daedalus-pty-pipeline)
