...
Either:
- **Neon**, create a project at neon.tech, get five database URLs (see [Database, Managed Neon](/docs/deploy/database/database-managed-neon)).
...
See [Security, Secrets Management](/docs/security/encryption-keys/secrets-management) for the full inventory.
...
Point `ciam.<domain>`, `iam.<domain>`, and `<domain>` (or `app.<domain>`) at your VPS IP. See [DNS and Domains](/docs/deploy/production/dns-and-domains).
...
- [ ] [Health endpoints](/docs/operate/monitoring/health-endpoints) all return 200.
...
- [ ] Set up the [cert-expiry alert](/docs/operate/rotation/cert-rotation).

Within a week:

- [ ] First [secrets audit](/docs/operate/monitoring/secrets-audit) cadence scheduled.
- [ ] [Backup procedure](/docs/operate/backups-recovery/backups-postgres) tested with a restore drill.
- [ ] Monitoring / log forwarding configured.

## Where next

- [Operate, Incident response](/docs/operate/backups-recovery/incident-response), when something breaks.
- [Operate, Network Topology](/docs/operate/administration/network-topology), firewall verification.
