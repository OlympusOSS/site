...
**Outputs:** 10+ secrets generated, stored locally in `daedalus.json`, then pushed to GitHub Actions Secrets via `gh secret set`. See [Security — Secrets Management](/docs/security/secrets-management) for the full inventory.
...
See [Integrate — MCP with Daedalus](/docs/integrate/mcp-with-daedalus) and [Internals — Daedalus MCP server](/docs/internals/daedalus-mcp-server).

## When the wizard isn't enough

If your deployment doesn't fit the wizard's assumptions (multi-region, K8s, custom DNS provider, etc.), use [Without the wizard — manual deploy](/docs/deploy/without-daedalus-manual).

## Where next

- [Deploy — First production deploy](/docs/deploy/first-production-deploy) — what's happening behind the wizard.
