...
For Postgres self-hosted on the same droplet, add at least 2GB RAM headroom. For [managed Postgres](/docs/deploy/database-managed-neon) (recommended), the droplet just runs the apps.
...
The admin API ports (`:3101`, `:3103`, `:4101`, `:4103`, `:5432`) are **not** exposed by the firewall — they're host-bound but inaccessible externally. See [Operate — Network Topology](/docs/operate/network-topology).
...
DigitalOcean offers automated weekly backups for droplets at +20% of the droplet cost. Useful for "oops I deleted something" recovery, but the better path is application-level backups: [Operate — Backups (Postgres)](/docs/operate/backups-postgres).

## What if you already have a droplet?

Use [Provider — Direct SSH](/docs/deploy/provider-direct-ssh) instead. Daedalus's DigitalOcean path provisions a new droplet; if you have an existing one, point Daedalus at its IP via the SSH path.
...
- [Deploy — DNS and Domains](/docs/deploy/dns-and-domains) — point your domain at the new droplet.
- [Deploy — Database (managed Neon)](/docs/deploy/database-managed-neon) — recommended Postgres path.
- [Deploy — First production deploy](/docs/deploy/first-production-deploy) — the full first-deploy walkthrough.
