...
**Recommendation:** prefer [managed Postgres (Neon)](/docs/deploy/database/database-managed-neon) unless one of the specific reasons below applies. The operational savings outweigh the cost.
...
- **TLS certs.** Generate, distribute, rotate (see [Operate, Cert Rotation](/docs/operate/rotation/cert-rotation)).
...
See [Operate, Backups (Postgres)](/docs/operate/backups-recovery/backups-postgres) for the full procedure including restore drill cadence.
...
See [Deploy, Database SSL verify-full](/docs/deploy/database/database-ssl-verify-full) for the full setup.

## Migrations

Olympus migrations run automatically, Kratos and Hydra each have their own migrate containers that run before the main service starts. For PostgreSQL major version upgrades, see [Operate, Logs and observability](/docs/operate/monitoring/logs-and-observability) for the recommended `pg_upgrade` procedure.
