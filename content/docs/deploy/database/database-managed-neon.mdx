...
The Neon CA bundle is fetched once during deployment and stored at `/etc/ssl/certs/neon-ca.crt` inside each app container. See [Deploy, Database SSL verify-full](/docs/deploy/database/database-ssl-verify-full) for the full rationale.
...
For belt-and-suspenders backups, run nightly `pg_dump` against each of the five databases, see [Operate, Backups (Postgres)](/docs/operate/backups-recovery/backups-postgres).
...
- **No internet egress from your VPS** (e.g. air-gapped deployments), use [self-hosted Postgres](/docs/deploy/database/database-self-hosted).
...
- [Deploy, Database SSL verify-full](/docs/deploy/database/database-ssl-verify-full), the production TLS contract.
