...
The `HERA_BASE_URL` environment variable must be set in the Athena container. See [oauth2-error-codes.md](/docs/integrate/oauth2-error-codes) for configuration details.
...
All failures are logged server-side before the redirect. Logs contain only `{ error_code, step, timestamp }` — no PII, no token values, no query strings. See [oauth2-error-codes.md](/docs/integrate/oauth2-error-codes) for the full error code reference, plain-English descriptions, and developer remediation steps.
