...
Fix: configure Kratos's cookie domain to a parent that covers both, see [Troubleshooting, Login loops](/docs/troubleshooting/auth-issues/login-loops).
...
- **Cross-domain CIAM ↔ IAM**: cookies set on `ciam.example.com` never travel to `iam.example.com`. This is intentional (see [CIAM/IAM isolation](/docs/security/infrastructure/ciam-iam-isolation)) but operators sometimes assume the parent domain bridges. Set the cookie domain explicitly per-side.
