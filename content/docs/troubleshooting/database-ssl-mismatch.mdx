...
If the CA cert has expired, every TLS handshake fails. Generate a new CA — see [Operate — Cert Rotation](/docs/operate/cert-rotation).
...
Generate new CA + server cert, deploy to Postgres, redeploy apps with the new CA bundle. See [Operate — Cert Rotation](/docs/operate/cert-rotation).
