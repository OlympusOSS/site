...
const DIRECTIVES = [
	...
	{ slug: "respond", name: "respond", purpose: "Return a static response without proxying.", olympusUse: "Returns 200 on /healthz; blocks specific paths early." },
	{ slug: "redir", name: "redir", purpose: "HTTP redirect.", olympusUse: "Force HTTP → HTTPS upgrade. Some legacy path redirects." },
	{ slug: "encode", name: "encode", purpose: "Response compression (gzip, zstd, br).", olympusUse: "Compresses responses by default for bandwidth savings." },
	{ slug: "log", name: "log", purpose: "Access logging.", olympusUse: "JSON-formatted access log to stdout for log shipping." },
	{ slug: "handle", name: "handle", purpose: "Route requests based on matcher; mutually exclusive in a host block.", olympusUse: "Per-path routing — `/.well-known/openid-configuration` → Hydra, `/login` → Hera, etc." },
	{ slug: "matcher", name: "@matcher", purpose: "Named request matcher referenced by handlers.", olympusUse: "Defines reusable matchers like `@login`, `@admin_api` for per-path policies." },
];
