...
const STEPS = [
	{
		...
		inputs: ["GitHub PAT with `repo` + `workflow` scopes"],
		outputs: ["Local clone at ~/code/YOUR-ORG-platform/", "Recorded fork URL in daedalus.json"],
	},
	{
		n: 2,
		slug: "domain",
		...
		inputs: ["Apex domain", "Subdomain names (defaults: ciam, iam, www)"],
		outputs: ["Edited prod kratos.yml, hydra.yml, Caddyfile with chosen domains"],
	},
	{
		n: 3,
		slug: "provider",
		...
		inputs: ["Provider (DigitalOcean / Hostinger / Direct SSH)", "Provider API token"],
		outputs: ["Recorded provider config"],
	},
	{
		n: 4,
		slug: "email",
		...
		inputs: ["Provider (Resend / Postmark / Brevo / SMTP2GO / AWS SES / Custom SMTP)", "API key or SMTP credentials", "Sender address"],
		outputs: ["SMTP settings in both Kratos configs"],
	},
	{
		n: 5,
		slug: "database",
		...
		outputs: ["5 DATABASE_URLs generated for the Olympus databases"],
	},
	{
		n: 6,
		slug: "compute",
		...
		outputs: ["VPS provisioned, firewall set, SSH key uploaded"],
	},
	{
		n: 7,
		slug: "secrets",
		...
		outputs: ["10+ secrets generated, stored locally and pushed to GitHub Actions Secrets"],
	},
	{
		n: 8,
		slug: "oauth",
		...
		inputs: ["None — auto-generated"],
		outputs: ["OAuth2 clients registered post-deploy via Hydra admin API"],
	},
	{
		n: 9,
		slug: "deploy",
		...
		inputs: ["None — runs based on prior steps"],
		outputs: ["GitHub Actions workflow triggered; containers running on VPS"],
	},
	{
		n: 10,
		slug: "health",
		...
		inputs: ["None"],
		outputs: ["Pass/fail per service health endpoint"],
	},
	{
		n: 11,
		slug: "accounts",
		...
		inputs: ["Admin email", "Admin password"],
		outputs: ["IAM admin identity created in Kratos"],
	},
	{
		n: 12,
		slug: "applications",
		...
		outputs: ["Stamped into Athena's apps settings"],
	},
	{
		n: 13,
		slug: "destroy",
		...
		inputs: ["Confirmation"],
		outputs: ["VPS deleted, DNS removed, Postgres dropped (managed), local context wiped"],
	},
];
...
writeFileSync(join(OUT_DIR, "meta.json"), JSON.stringify({ title: "Wizard steps", pages: ["overview", ...STEPS.map((s) => s.slug)] }, null, 2));
