 ...
 */
export function buildAuthUrl(domain: OAuthDomain, state: string): string {
	...
	const clientId = domain === "ciam" ? process.env.CIAM_CLIENT_ID || "site-ciam-client" : process.env.IAM_CLIENT_ID || "site-iam-client";
 ...
 */
export async function handleOAuthLogout(request: NextRequest, domain: OAuthDomain): Promise<NextResponse> {
	...
	if (subject) {
		await Promise.allSettled([
			fetch(`${config.hydraAdminUrl}/admin/oauth2/auth/sessions/login?subject=${encodeURIComponent(subject)}`, { method: "DELETE" }).catch((err) =>
				console.error(`[${domain}] Hydra login session revoke failed:`, err),
			),
			fetch(`${config.hydraAdminUrl}/admin/oauth2/auth/sessions/consent?subject=${encodeURIComponent(subject)}&all=true`, { method: "DELETE" }).catch(
				(err) => console.error(`[${domain}] Hydra consent session revoke failed:`, err),
			),
			fetch(`${config.kratosAdminUrl}/admin/identities/${encodeURIComponent(subject)}/sessions`, { method: "DELETE" }).catch((err) =>
				console.error(`[${domain}] Kratos session revoke failed:`, err),
			),
		]);
	}
 ...
 */
export async function handleOAuthCallback(request: NextRequest, domain: OAuthDomain): Promise<NextResponse> {
	const config = getDomainConfig(domain);
	const appUrl = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:2000";
	...
	if (!storedState || !returnedState || storedState !== returnedState) {
		console.error(`${config.errorPrefix.toUpperCase()} OAuth state mismatch — possible CSRF attack`);
		return NextResponse.redirect(new URL(`/?error=${config.errorPrefix}_state_mismatch#playground`, appUrl));
	}
	...
	try {
		...
		if (!tokenRes.ok) {
			...
			return NextResponse.redirect(new URL(`/?error=${config.errorPrefix}_token_exchange_failed#playground`, appUrl));
		}
		...
		return NextResponse.redirect(new URL(`/?error=${config.errorPrefix}_callback_failed#playground`, appUrl));
	}
}
